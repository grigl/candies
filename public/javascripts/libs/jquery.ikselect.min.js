 	...
 	customSelect : function(options) {
	  ...
	  return this.each(function() {
			...
			var sortedVals = $.makeArray($(this).find('option')).sort(function(a,b){
					return parseFloat($(a).text()) > parseFloat($(b).text()) ? 1 : parseFloat($(a).text()) < parseFloat($(b).text()) ? -1 : 0 ;
			});
			$(this).empty().html(sortedVals);
			var $selectedElem = currentSelected.length ? $(this).find('[value="' + currentSelected.val().replace(/\\/g, '\\\\') + '"]') : $(this).find(':first');
			...
			var old_value = $(this).find(':selected').text();
